...
$dbname3 = "another_report";
$dbname4 = "yet_another_report";
...
if ($conn3->connect_error) {
    die("Connection to another_report database failed: " . $conn3->connect_error);
}
if ($conn4->connect_error) {
    die("Connection to yet_another_report database failed: " . $conn4->connect_error);
}
...
if (isset($_POST['delete_another_report'])) {
    $id = $_POST['delete_another_report'];
    $deleteStmt = $conn3->prepare("DELETE FROM another_report WHERE id = ?");
    $deleteStmt->bind_param("i", $id);
    $deleteStmt->execute();
    $deleteStmt->close();
}

if (isset($_POST['delete_yet_another_report'])) {
    $id = $_POST['delete_yet_another_report'];
    $deleteStmt = $conn4->prepare("DELETE FROM yet_another_report WHERE id = ?");
    $deleteStmt->bind_param("i", $id);
    $deleteStmt->execute();
    $deleteStmt->close();
}
...
$anotherReports = $conn3->query("SELECT * FROM another_report");
$yetAnotherReports = $conn4->query("SELECT * FROM yet_another_report");
?>
